package com.joker.javase.thread;
 ...
 *  实现多线程的时候：
 *      1.需要继承Thread类
 *      2.必须要重写run方法，指的是 核心执行的逻辑
 *      3.线程在启动的时候，不要直接调用run方法，而是通过start来进行调用
 *      4.每次运行相同的代码，出来的结果可能不一样，原因在于多线程的执行无法人为控制
 *  第二种实现方式：使用了代理设计模式
 *      1、实现Runnable接口
 *      2、重写run方法
 ...
 *              在执行的过程中就叫做运行状态 抢占到cpu资源，执行代码逻辑开始
